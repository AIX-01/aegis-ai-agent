...
def _patch_resolution(data: bytearray, width: int, height: int) -> bytearray:
    ...
    PyAV remux 모드에서 avc1에 해상도가 0으로 설정되는 문제를 해결합니다.
    """
    # avc1 패치 (offset 32-33: width, 34-35: height)
    ...
    # tkhd 패치 (width/height는 fixed-point 16.16)
    tkhd_idx = data.find(b'tkhd')
    if tkhd_idx > 0:
        # version 0 기준: offset 76-79: width, 80-83: height
...
def _faststart(data: bytes, width: int = 0, height: int = 0) -> bytes:
    ...
    # moov가 이미 앞에 있으면 edts 제거와 호환성 처리만
...
def mux_packets_to_mp4(
    ...
    try:
        with av.open(buf, mode='w', format='mp4') as container:
            ...
            # codec_context에도 명시적 설정 (avc1 박스에 반영)
