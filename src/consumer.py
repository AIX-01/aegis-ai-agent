...
class ConsumerPool:
    ...
    1. 저해상도 → VLM 분석
    2. VLM 결과 → 백엔드 전송
    3. VLM 결과가 '이상'이면 → 저해상도 → 정밀 분석 API 전송
    """
    ...
    def __init__(
        ...
        Args:
            ...
            precision_client: 정밀 분석 클라이언트 (3단계)
            backend_client: 백엔드 API 클라이언트 (2단계)
        """
        self.config = config
        self.queue_manager = queue_manager
        self.vlm_client = vlm_client
        self.precision_client = precision_client
        self.backend_client = backend_client # 백엔드 클라이언트 할당
    ...
    def _worker_loop(self, worker_id: int):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                # === 2단계: VLM 결과를 백엔드로 전송 ===
                self.backend_client.send_vlm_result(camera_id, vlm_result, task)

                # === 3단계: 트리거 조건 확인 및 정밀 분석 전송 ===
                primary_category = vlm_result.get("primary_category", "").lower()
                is_abnormal = any(cat in primary_category for cat in TRIGGER_CATEGORIES)

                if is_abnormal:
                    self.total_abnormal += 1
                    worker_logger.info(
                        f"[이상 감지] {camera_id}의 조건 충족. 정밀 분석으로 전송합니다."
                    )

                    if low_res_frames:
                        precision_result = self.precision_client.send_for_analysis(
                            camera_id, low_res_frames, vlm_result, task
                        )
                        if precision_result:
                        ...
                        worker_logger.warning(f"{camera_id}에 전송할 저해상도 프레임이 없습니다")
                else:
                    self.total_normal += 1
                    worker_logger.debug(f"[정상] {camera_id} - 정밀 분석 불필요")
