...
외부 관리 서버(예: Spring Boot)가 Redis에 등록한 카메라 목록을 동적으로 관리하며, VLM 분석 결과를 백엔드로 전송하고, '이상' 조건 충족 시에만 **저해상도 프레임**을 정밀 분석 API로 즉시 전송하는 경량화된 파이프라인 시스템입니다.
...
1단계 [VLM 분석]  → 저해상도 → VLM 분석 ┐
                                    ...
                                    └→ '이상' 판정 ┐
                                                 ↓ (조건 충족)
2단계 [정밀 분석] → 저해상도 → 정밀 API 즉시 전송 → 상세 분석
```

### 핵심 개념
- **VLM 결과 전송**: 모든 VLM 분석 결과(정상, 이상 등)를 실시간으로 백엔드 서버에 전송하여 모니터링 및 통계에 활용합니다.
- **조건부 정밀 분석**: VLM이 "이상(abnormal)" 카테고리를 반환할 경우에만, 2단계 정밀 분석을 진행하여 자원을 효율적으로 사용합니다.
- **Redis 동적 스트림 관리**: 에이전트 재시작 없이 외부 관리 서버가 Redis의 설정을 변경하는 것만으로 분석할 카메라 스트림을 실시간으로 추가하거나 제거할 수 있습니다.

## 주요 기능

### 🎯 핵심 기능
- **VLM 결과 백엔드 전송**: 모든 1차 분석 결과를 지정된 백엔드 서버로 전송합니다.
- **조건부 2단계 파이프라인**: VLM 분석 후 '이상' 조건 충족 시에만 정밀 분석으로 전환됩니다.
...
graph TD
    ...
    C -- "11. 모든 VLM 결과 백엔드 전송" --> Backend
    C -- "12. 트리거 조건 판단" --> Decision{"'이상'인가?"}
    Decision -- "No (정상 또는 의심)" --> C_Normal("13a. 작업 종료")
    Decision -- "Yes (이상)" --> C_Abnormal
    C_Abnormal -- "13b. 정밀 분석 요청" --> Precision
```
**워크플로우 설명:**
...
12. **분기 처리**: `Consumer`는 VLM 결과가 설정된 트리거 조건(예: 'abnormal', '이상')에 해당하는지 확인합니다.
    *   **정상 또는 의심일 경우 (13a)**: 정밀 분석 없이 작업을 종료합니다.
    *   **이상일 경우 (13b)**: `정밀 분석 서버`로 2차 분석을 요청합니다.
...
| **Backend Client** (`backend_client.py`) | 백엔드 서버와의 HTTP 통신 담당 |
